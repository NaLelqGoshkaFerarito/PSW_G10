...
This is over a time period of 1 week. (current week)
"""
def plot_week(metric, sensor):
    ...
    if metric == "Temperature":
        ...
        if sensor == "py-saxion" or sensor == "py-wierden" or sensor == "py-gronau" or sensor == "py-group9" or sensor == "lht-saxion":
            cursor.execute(
                "SELECT temp_in, time from status where time > DATE_SUB(CURRENT_DATE(), INTERVAL 1 week) AND device_id = %s",
                (sensor,))
        ...
        # All other sensors read an outside temperature
            ...
            cursor.execute(
                "SELECT temp_out, time from status where time > DATE_SUB(CURRENT_DATE(), INTERVAL 1 week) AND device_id = %s",
                (sensor,))
            result = cursor.fetchall()
            for r in result:
                temp.append(r[0])
        ...
        if sensor == 'lht-saxion':
            cursor.execute(
                "SELECT temp_out, time from status where time > DATE_SUB(CURRENT_DATE(), INTERVAL 1 week) AND device_id = 'lht-saxion'")
            result = cursor.fetchall()
            for r in result:
    ...
    if metric == "Humidity":
        ...
        cursor.execute(
            "SELECT humidity, time from status where time > DATE_SUB(CURRENT_DATE(), INTERVAL 1 week) AND device_id = %s",
            (sensor,))
        result = cursor.fetchall()
        for r in result:
            humidity.append(r[0])
