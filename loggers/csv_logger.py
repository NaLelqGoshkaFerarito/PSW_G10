# logs to a CSV file; was used before the db_logger was implemented
# the database code had to scan every so often, which was not very efficient
    ...
    def log(self, data):
        ...
        if type(data) == PYData:
            with open("./loggers/logs_csv/" + self.__file_name, "a") as file:
                ...
                writer.writerow(
                    [data.device_id, data.latitude, data.longitude, data.altitude, data.light, data.temperature,
                     data.pressure, data.datetime, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), data.consumed_airtime])
